...
function addExportBox(target, w, h, d, cx, cy, cz, fallbackBounds, faceBounds = {}) {
	target.push({
		minX: cx - w / 2,
		maxX: cx + w / 2,
		minY: cy - h / 2,
...
function rangesCover(min, max, start, end, eps = 1e-5) {
	return min <= start + eps && max >= end - eps;
}
function isNearlyEqual(a, b, eps = 1e-5) {
	return Math.abs(a - b) <= eps;
}
function pushSolidQuad(positions, normals, uvs, faceIndex, bounds, x0, x1, y0, y1, z0, z1) {
	const uCenter = (bounds.uMin + bounds.uMax) / 2;
	const vCenter = (bounds.vMin + bounds.vMax) / 2;
	const normalByFace = {
		0: [
	...
	const normal = normalByFace[faceIndex];
	for (const vertex of quadByFace[faceIndex]) {
		positions.push(vertex[0], vertex[1], vertex[2]);
		normals.push(normal[0], normal[1], normal[2]);
		uvs.push(uCenter, vCenter);
	}
}
function buildExteriorGeometryFromBoxes(boxes) {
	const xCoords = Array.from(new Set(boxes.flatMap((box) => [box.minX, box.maxX]))).sort((a, b) => a - b);
	const yCoords = Array.from(new Set(boxes.flatMap((box) => [box.minY, box.maxY]))).sort((a, b) => a - b);
	const zCoords = Array.from(new Set(boxes.flatMap((box) => [box.minZ, box.maxZ]))).sort((a, b) => a - b);
	const xCount = xCoords.length - 1;
	const yCount = yCoords.length - 1;
	const zCount = zCoords.length - 1;
	const occupied = new Uint8Array(xCount * yCount * zCount);
	const cellIndex = (ix, iy, iz) => (ix * yCount + iy) * zCount + iz;
	for (let ix = 0; ix < xCount; ix++) {
		const xMid = (xCoords[ix] + xCoords[ix + 1]) / 2;
		for (let iy = 0; iy < yCount; iy++) {
			const yMid = (yCoords[iy] + yCoords[iy + 1]) / 2;
			for (let iz = 0; iz < zCount; iz++) {
				const zMid = (zCoords[iz] + zCoords[iz + 1]) / 2;
				const inside = boxes.some((box) => xMid > box.minX + 1e-5 && xMid < box.maxX - 1e-5 && yMid > box.minY + 1e-5 && yMid < box.maxY - 1e-5 && zMid > box.minZ + 1e-5 && zMid < box.maxZ - 1e-5);
				occupied[cellIndex(ix, iy, iz)] = inside ? 1 : 0;
			}
		}
	}
	const findSourceBox = (faceIndex, plane, a0, a1, b0, b1) => {
		const matches = boxes.filter((box) => {
			switch (faceIndex) {
				case 0: return isNearlyEqual(box.maxX, plane) && rangesCover(box.minY, box.maxY, a0, a1) && rangesCover(box.minZ, box.maxZ, b0, b1);
				case 1: return isNearlyEqual(box.minX, plane) && rangesCover(box.minY, box.maxY, a0, a1) && rangesCover(box.minZ, box.maxZ, b0, b1);
				case 2: return isNearlyEqual(box.maxY, plane) && rangesCover(box.minX, box.maxX, a0, a1) && rangesCover(box.minZ, box.maxZ, b0, b1);
				case 3: return isNearlyEqual(box.minY, plane) && rangesCover(box.minX, box.maxX, a0, a1) && rangesCover(box.minZ, box.maxZ, b0, b1);
				case 4: return isNearlyEqual(box.maxZ, plane) && rangesCover(box.minX, box.maxX, a0, a1) && rangesCover(box.minY, box.maxY, b0, b1);
				case 5: return isNearlyEqual(box.minZ, plane) && rangesCover(box.minX, box.maxX, a0, a1) && rangesCover(box.minY, box.maxY, b0, b1);
				default: return false;
			}
		});
		matches.sort((a, b) => {
			const aExplicit = a.faceBounds[faceIndex] ? 1 : 0;
			const bExplicit = b.faceBounds[faceIndex] ? 1 : 0;
			if (aExplicit !== bExplicit) return bExplicit - aExplicit;
			return (a.maxX - a.minX) * (a.maxY - a.minY) * (a.maxZ - a.minZ) - (b.maxX - b.minX) * (b.maxY - b.minY) * (b.maxZ - b.minZ);
		});
	...
	const normals = [];
	const uvs = [];
	for (let ix = 0; ix < xCount; ix++) for (let iy = 0; iy < yCount; iy++) for (let iz = 0; iz < zCount; iz++) {
		if (!occupied[cellIndex(ix, iy, iz)]) continue;
		const x0 = xCoords[ix];
		const x1 = xCoords[ix + 1];
		const y0 = yCoords[iy];
		const y1 = yCoords[iy + 1];
		const z0 = zCoords[iz];
		const z1 = zCoords[iz + 1];
		const faceDefs = [
			{
				faceIndex: 0,
				visible: ix === xCount - 1 || !occupied[cellIndex(ix + 1, iy, iz)],
				plane: x1,
				a0: y0,
				a1: y1,
				b0: z0,
			...
			{
				faceIndex: 1,
				visible: ix === 0 || !occupied[cellIndex(ix - 1, iy, iz)],
				plane: x0,
				a0: y0,
				a1: y1,
				b0: z0,
			...
			{
				faceIndex: 2,
				visible: iy === yCount - 1 || !occupied[cellIndex(ix, iy + 1, iz)],
				plane: y1,
				a0: x0,
				a1: x1,
				b0: z0,
			...
			{
				faceIndex: 3,
				visible: iy === 0 || !occupied[cellIndex(ix, iy - 1, iz)],
				plane: y0,
				a0: x0,
				a1: x1,
				b0: z0,
			...
			{
				faceIndex: 4,
				visible: iz === zCount - 1 || !occupied[cellIndex(ix, iy, iz + 1)],
				plane: z1,
				a0: x0,
				a1: x1,
				b0: y0,
			...
			{
				faceIndex: 5,
				visible: iz === 0 || !occupied[cellIndex(ix, iy, iz - 1)],
				plane: z0,
				a0: x0,
				a1: x1,
				b0: y0,
		...
		for (const faceDef of faceDefs) {
			if (!faceDef.visible) continue;
			const sourceBox = findSourceBox(faceDef.faceIndex, faceDef.plane, faceDef.a0, faceDef.a1, faceDef.b0, faceDef.b1);
			if (!sourceBox) continue;
			const uvBounds = sourceBox.faceBounds[faceDef.faceIndex] ?? sourceBox.fallbackBounds;
			pushSolidQuad(positions, normals, uvs, faceDef.faceIndex, uvBounds, x0, x1, y0, y1, z0, z1);
		}
	}
...
function removeExactDuplicateTriangles$1(geometry) {
...
*/
function buildVoxelizedOverlayWithReliefInternal(skinImage, heightmap, boundaryInset = 0, exportSurfaceOnly = false) {
	...
	const baseBoundary = 4 + boundaryInset;
	let offsets = {
		right: baseBoundary,
		left: baseBoundary,
		top: baseBoundary,
	...
	const THICKNESS = .35;
	const CORNER_CLIP_W = .625 + boundaryInset;
	const CORNER_SHIFT = -.25 + boundaryInset / 2;
	const vcDefs = [
		{
			faceA: "front",
			colA: 7,
	...
	faces.forEach((face) => {
		for (let row = 0; row < 8; row++) for (let col = 0; col < 8; col++) {
			...
			if (vcr) {
				...
				if ((overlayMask[adjFaceKey]?.[row]?.[adjCol])?.active) {
					...
					if (!emittedCaps.has(vcCapKey)) {
						...
						if (exportSurfaceOnly) addExportBox(exportBoxes, THICKNESS, capH, THICKNESS, capPos.x, capPos.y, capPos.z, cpi, capFaceBounds);
						else {
				...
				if (heRole !== null) {
					...
					if (!emittedCaps.has(heCapKey)) {
						...
						if (piA?.active && piB?.active) {
							...
							if (exportSurfaceOnly) addExportBox(exportBoxes, cW, hDef.capH, cD, cPos.x, cPos.y, cPos.z, cpi, capFaceBounds);
							else {
			...
			if (exportSurfaceOnly) {
				...
				addExportBox(exportBoxes, voxelW, voxelH, THICKNESS, pos.x, pos.y, pos.z, fallbackBounds);
			} else {
				const geom = buildBoxGeometry$1(voxelW, voxelH, THICKNESS, info.uMin, info.uMax, info.vMin, info.vMax);
	...
	for (const tc of [
		...
		if (i1?.active && i2?.active && i3?.active) {
			...
			const tcX = tc.sx * (xOfs + THICKNESS / 2);
			const tcY = tc.sy * (yOfs + THICKNESS / 2);
			const tcZ = tc.sz * (zOfs + THICKNESS / 2);
			if (exportSurfaceOnly) addExportBox(exportBoxes, THICKNESS, THICKNESS, THICKNESS, tcX, tcY, tcZ, i1, tcFaceBounds);
			else {
				const tcFaceGeom = buildBoxGeometryWithFacePixels(THICKNESS, THICKNESS, THICKNESS, i1, tcFaceBounds);
				tcFaceGeom.translate(tcX, tcY, tcZ);
	...
	if (exportSurfaceOnly ? exportBoxes.length > 0 : geometries.length > 0) {
		const mergedGeom = exportSurfaceOnly ? buildExteriorGeometryFromBoxes(exportBoxes) : removeExactDuplicateTriangles$1(mergeBufferGeometries$1(geometries));
		if (!exportSurfaceOnly) geometries.forEach((g) => g.dispose());
...
function buildVoxelizedOverlayWithRelief(skinImage, heightmap) {
	return buildVoxelizedOverlayWithReliefInternal(skinImage, heightmap, 0, false);
}
function buildVoxelizedOverlayWithReliefForExport(skinImage, heightmap, boundaryInset = .02) {
	return buildVoxelizedOverlayWithReliefInternal(skinImage, heightmap, boundaryInset, true);
}
/**
* Builds the Minecraft Head 3D model (Group containing Head and HeadOverlay meshes).
