...
function buildVoxelizedOverlayWithRelief(skinImage, heightmap) {
	...
	if (geometries.length > 0) {
		const mergedGeom = removeExactDuplicateTriangles$1(mergeBufferGeometries$1(geometries));
...
function createExportSafeReliefHeightmap(heightmap, epsilon = .02) {
	const baseOffsets = {
		right: 4,
		left: 4,
		top: 4,
	...
	const sourceOffsets = heightmap?.offsets ? {
		...baseOffsets,
		...heightmap.offsets
	} : baseOffsets;
	return {
		...heightmap,
		offsets: {
			right: sourceOffsets.right + epsilon,
			left: sourceOffsets.left + epsilon,
			top: sourceOffsets.top + epsilon,
			bottom: sourceOffsets.bottom + epsilon,
			front: sourceOffsets.front + epsilon,
			back: sourceOffsets.back + epsilon
		}
	};
}
function buildReliefExportGroup(skinImage, heightmap) {
	...
	const reliefOverlay = buildVoxelizedOverlayWithRelief(skinImage, createExportSafeReliefHeightmap(heightmap));
